...
describe('buildDailyQuiz', () => {
  ...
  describe('detectCategory', () => {
    it('detects politics from election keywords', () => {
      expect(detectCategory(makeArticle('President election results', 0))).toBe('politics');
    });

    it('detects tech from software keywords', () => {
      expect(detectCategory(makeArticle('New AI app launches tech revolution', 1))).toBe('tech');
    });

    it('detects sports from team and game keywords', () => {
      expect(detectCategory(makeArticle('Team wins championship game', 2))).toBe('sports');
    });

    it('detects science from research keywords', () => {
      expect(detectCategory(makeArticle('Scientists make discovery in space', 3))).toBe('science');
    });

    it('detects world from international keywords', () => {
      expect(detectCategory(makeArticle('International crisis summit held', 4))).toBe('world');
    });

    it('detects business from market keywords', () => {
      expect(detectCategory(makeArticle('Stock market economy inflation', 6))).toBe('business');
    });

    it('falls back to general when no keywords match', () => {
      expect(detectCategory(makeArticle('Local cat found in tree', 7))).toBe('general');
    });
    ...
    it('uses the highest-scoring category when multiple match', () => {
      ...
      expect(detectCategory(makeArticle('President election and AI technology', 8))).toBe('tech');
    });
  });
