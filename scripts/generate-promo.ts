...
.source-url{font-size:8px;color:#6b6b6b;word-break:break-all;margin-top:4px}
...
type Style = "classic" | "hero" | "splash";
...
const STYLES: Record<Style, string> = {
  classic: `
    ...
    .hero-title-text{font-size:52px;margin-top:4px}.tagline{display:none}
  ...
  splash: `
    ...
    .splash-header{display:flex;align-items:center;gap:10px;align-self:flex-start;margin-bottom:16px}
    .masthead{font-size:22px;font-family:"Space Grotesk",sans-serif;font-weight:900;text-transform:uppercase;letter-spacing:.16em}
    .hero-title-text{display:none}.tagline{display:none}.kicker{display:none}
...
async function renderOne(
  ...
  const styles: Style[] = ["classic", "hero", "splash"];
  const paths: string[] = [];
