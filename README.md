...
**Static site**: Vercel (auto-deploys on push to main). No server needed at runtime — the quiz data lives in `public/quizzes.json` and gets baked into the Vite build.

**Daily cron**: GitHub Actions runs `pnpm generate-quiz --fresh`, commits the updated quiz data, and pushes — Vercel picks up the new commit and redeploys automatically.

**Promo storage**: Cloudflare R2 — zero-egress object storage for promo images generated by the cron job.

### One-time setup

1. **Vercel**: Import repo, framework=Vite, build command=`pnpm build`, output dir=`dist`
2. **GitHub Secrets**: `NEWSAPI_KEY`, `DEEPSEEK_API_KEY`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`
3. **Cloudflare R2**: Create bucket `newser-promos`, generate API token with R2 read/write, add to GitHub Secrets
